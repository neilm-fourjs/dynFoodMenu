...
-- Contains methods to retrieve the menu data from JSON / DB / WebService
IMPORT util
IMPORT os
IMPORT FGL appInfo
IMPORT FGL debug
...
PUBLIC FUNCTION (this Menus) getMenu(l_menuName STRING) RETURNS BOOLEAN
	...
	IF libMobile.gotNetwork() THEN
		IF NOT this.getMenuWS(l_menuName) THEN
			RETURN FALSE
		END IF
	ELSE
		IF NOT this.getMenuJSON(l_menuName) THEN
			RETURN FALSE
		END IF
	END IF
...
PUBLIC FUNCTION (this Menus) placeOrderDB() RETURNS(INTEGER, STRING)
	...
	DEFINE x     SMALLINT
	IF NOT g_db.connect() THEN
		RETURN 100, "No database connection1"
	END IF
-- Store order in DB
...
FUNCTION (this Menus) getMenuListWS() RETURNS BOOLEAN
	...
	LET wsMenus.Endpoint.Address.Uri = g_wsAuth.getWSServer(appInfo.appInfo.ws_menus)
...
FUNCTION (this Menus) getMenuWS(l_menuName STRING) RETURNS BOOLEAN
	...
	END IF
	CALL debug.output(
			SFMT("getMenuWS: %1 Rows: %2 from %3", l_menuName, this.menu.rows, wsMenus.Endpoint.Address.Uri), FALSE)
...
FUNCTION (this Menus) save()
	...
	CALL debug.output(
			SFMT("save Stat:%1:%2:%3 - from %4", l_stat, l_resp.l_stat, l_resp.l_msg, wsMenus.Endpoint.Address.Uri), FALSE)
