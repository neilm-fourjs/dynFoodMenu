		...
		AFTER INPUT
			IF NOT int_flag THEN
				...
				CASE g_wsAuth.cfg.ServiceVersion
					WHEN "v1"
						CALL wsUsers.v1_getUser(this.currentUser.user_id, utils.apiPaas(this.currentUser.user_id, this.server_time))
								RETURNING l_stat, this.currentUser.*
					WHEN "v2"
						CALL wsUsers.v2_getUser(
										this.currentUser.user_id, utils.apiPaas(this.currentUser.user_id, this.server_time), NULL, NULL)
								RETURNING l_stat, this.currentUser.*
				END CASE
				CALL debug.output(
						SFMT("getUser: %1, reply: %2 : %3(%4)",
								this.currentUser.user_id, l_stat, this.currentUser.user_name, this.currentUser.user_pwd),
						FALSE)
...
-- Get access token if we don't already have one.
	IF g_wsAuth.cfg.GAS IS NULL THEN
		CALL debug.output(
				SFMT("Doing g_wsAuth.init( '%1', '%2', '%3'", g_cfg.cfgDir, g_cfg.wsCFGFile, g_cfg.wsCFGName), FALSE)
...
-- Get the servers timestamp.
	CALL debug.output(
			SFMT("Getting timestamp api: '%1' uri: %2", g_wsAuth.cfg.ServiceVersion, wsUsers.Endpoint.Address.Uri), FALSE)
	CASE g_wsAuth.cfg.ServiceVersion
		WHEN "v1"
			CALL wsUsers.v1_getTimeStamp() RETURNING l_stat, this.server_time
		WHEN "v2"
