...
FUNCTION (this db) create_tabs()
	...
	CREATE TABLE dbver(dbver INTEGER)
	INSERT INTO dbver VALUES(1)

	CALL debug.output("Create table: users", FALSE)
	CREATE TABLE users(
			user_id VARCHAR(6), user_name VARCHAR(30), user_pwd VARCHAR(60), user_token VARCHAR(60),
			token_ts DATETIME YEAR TO MINUTE)

	CALL debug.output("Create table: userDetails", FALSE)
	CREATE TABLE userDetails(
			user_id VARCHAR(6), salutation VARCHAR(30), firstnames VARCHAR(30), surname VARCHAR(30), email VARCHAR(50),
			dob DATE, gender_preference CHAR(1), password_hash VARCHAR(60), registered DATETIME YEAR TO MINUTE)

	CALL debug.output("Create table: menus", FALSE)
	CREATE TABLE menus(menuName VARCHAR(6), menuDesc VARCHAR(30), menuImage VARCHAR(20))

	CALL debug.output("Create table: menuItems", FALSE)
	CREATE TABLE menuItems(
			menuName VARCHAR(6), t_id INTEGER, t_pid INTEGER, id VARCHAR(6), type VARCHAR(10), description VARCHAR(30),
			conditional BOOLEAN, minval INTEGER, maxval INTEGER, field VARCHAR(25), option_id VARCHAR(6),
			option_name VARCHAR(30), hidden BOOLEAN, level SMALLINT)

	CALL debug.output("Create table: orders", FALSE)
	CREATE TABLE orders(
			order_id SERIAL, user_token VARCHAR(60), user_id VARCHAR(6), patient_id INTEGER, ward_id INTEGER, bed_no SMALLINT,
			menu_id VARCHAR(6), placed DATETIME YEAR TO SECOND)

	CALL debug.output("Create table: orderItems", FALSE)
	CREATE TABLE orderItems(order_id INTEGER, item_id INTEGER, description VARCHAR(30), qty SMALLINT, optional BOOLEAN)

	CALL debug.output("Create table: wards", FALSE)
	CREATE TABLE wards(ward_id INTEGER, ward_name VARCHAR(30))

	CALL debug.output("Create table: patients", FALSE)
	CREATE TABLE patients(
			id SERIAL, name VARCHAR(40), dob DATE, gender_preference CHAR(1), ward_id INTEGER, bed_no SMALLINT,
			nilbymouth BOOLEAN, diabetic BOOLEAN, allergies VARCHAR(100))
